...
import { useDoctors, useDoctorActions } from "@/hooks/useDoctors";
import { DoctorCard } from "@/components/doctors/DoctorCard";
import { DoctorsFilters } from "@/components/doctors/DoctorsFilters";
import { DoctorModal } from "@/components/doctors/DoctorModal";
import { DoctorConfirmModal } from "@/components/doctors/DoctorConfirmModal";
import { DoctorDetailsModal } from "@/components/doctors/DoctorDetailsModal";
import Button from "@/components/ui/button/Button";
import type { Doctor, DoctorFilters } from "@/types/doctors";
...
export function DoctorsList() {
  ...
  const { activate, deactivate, loading: actionLoading } = useDoctorActions();
  ...
  const [actioningDoctor, setActioningDoctor] = useState<string | null>(null);
  ...
  const handleToggleActive = (doctor: Doctor) => {
    setConfirmAction({
      doctor,
      action: doctor.isActive ? "deactivate" : "activate",
    });
  };
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      setActioningDoctor(confirmAction.doctor.id);
      if (confirmAction.action === "activate") {
        await activate(confirmAction.doctor.id);
      } else {
        await deactivate(confirmAction.doctor.id);
      }
      refresh();
    } catch (error) {
  ...
  const handleFiltersChange = (newFilters: Partial<DoctorFilters>) => {
  ...
  const renderTableRow = (doctor: Doctor) => (
    <tr
      key={doctor.id}
    ...
    >
      <td className="px-6 py-4">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-brand-100 dark:bg-brand-900">
            {doctor.profilePhoto ? (
              ...
              <span className="text-sm font-semibold text-brand-600 dark:text-brand-400">
                {doctor.user
                  ? `${doctor.user.name.charAt(0)}${doctor.user.lastName.charAt(
                      0
                    )}`
                  : "NA"}
              </span>
          ...
          <div>
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400">
              {doctor.user?.email || "Email no disponible"}
            </div>
          </div>
        </div>
      </td>
      ...
      <td className="px-6 py-4">
        <div className="text-sm text-gray-900 dark:text-gray-100">
          {doctor.user?.phone || "No disponible"}
        </div>
      </td>
      ...
      <td className="px-6 py-4">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={actioningDoctor === doctor.id}
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-12 text-center dark:border-gray-800 dark:bg-gray-900">
          ...
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
            ...
            filters.isActive !== undefined
              ...
              : "Comienza registrando el primer doctor en la clínica."}
          </p>
          {!filters.search &&
            !filters.speciality &&
            filters.isActive === undefined && (
            ...
            <div className="grid gap-6">
              {doctors.map((doctor: Doctor) => (
                <DoctorCard
                  ...
                  loading={actioningDoctor === doctor.id}
                />
              ))}
            </div>
          </div>
      ...
      {detailsDoctor && (
        <DoctorDetailsModal
          isOpen={!!detailsDoctor}
          doctor={detailsDoctor}
          onClose={handleDetailsClose}
          onEdit={() => {
            handleDetailsClose();
            handleEdit(detailsDoctor);
          }}
        />
      )}
