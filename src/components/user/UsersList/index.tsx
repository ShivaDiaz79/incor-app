...
import { UsersFilters } from "@/components/user/UserFilter";
...
import type { User, UserFilters, ChangePasswordRequest } from "@/types/users";

export function UsersList() {
  const { users, loading, error, pagination, filters, updateFilters, refresh } =
    useUsers({
      page: 1,
      limit: 20,
      sortBy: "createdAt",
      sortOrder: "desc",
      includeRoles: true, // Incluir detalles de roles
    });
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      setActioningUser(confirmAction.user.id);
      if (confirmAction.action === "activate") {
        await activate(confirmAction.user.id);
      } else {
        await deactivate(confirmAction.user.id);
      }
      refresh();
    } catch (error) {
  ...
  const handlePasswordConfirm = async (passwords: ChangePasswordRequest) => {
    if (!passwordUser) return;

    try {
      await changePassword(passwordUser.id, passwords);
          ...
          <div className="flex flex-wrap gap-1">
            ...
            {(!user.roleDetails || user.roleDetails.length === 0) &&
              (!user.roleIds || user.roleIds.length === 0) && (
                <span className="inline-flex rounded-full px-2 py-1 text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300">
                  Sin roles
                </span>
              )}
          </div>
    ...
    <div className="space-y-6">
      ...
      {detailsUser && (
        <UserDetailsModal
          ...
          onChangePassword={() => {
            handleDetailsClose();
            handleChangePassword(detailsUser);
          }}
        />
      )}
