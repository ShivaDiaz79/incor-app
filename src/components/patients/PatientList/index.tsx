...
import type { Patient, PatientFilters } from "@/types/patients";
...
export function PatientsList() {
  ...
  const { activate, deactivate, loading: actionLoading } = usePatientActions();

  const [selectedPatient, setSelectedPatient] = useState<Patient | null>(null);
  const [modalMode, setModalMode] = useState<"create" | "edit" | null>(null);
  const [detailsPatient, setDetailsPatient] = useState<Patient | null>(null);
  const [actioningPatient, setActioningPatient] = useState<string | null>(null);
  ...
  const handleToggleActive = async (patient: Patient) => {
    try {
      setActioningPatient(patient.id);
      if (patient.isActive) {
        await deactivate(patient.id);
      } else {
        await activate(patient.id);
      }
      refresh();
    } catch (error) {
      console.error("Error toggling patient status:", error);
    } finally {
      setActioningPatient(null);
    }
  };
      ...
      <td className="px-6 py-4">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={actioningPatient === patient.id}
      ...
      {!loading && !error && patients.length === 0 && (
        <div className="rounded-2xl border border-gray-200 bg-white p-12 text-center dark:border-gray-800 dark:bg-gray-900">
          ...
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
            {filters.search || filters.gender || filters.city
              ? "No se encontraron pacientes que coincidan con los filtros."
              : "Comienza registrando tu primer paciente en la clínica."}
          </p>
          {!filters.search && !filters.gender && !filters.city && (
            ...
            <div className="grid gap-6">
              {patients.map((patient: Patient) => (
                <PatientCard
                  ...
                  loading={actioningPatient === patient.id}
                />
              ))}
            </div>
          </div>
